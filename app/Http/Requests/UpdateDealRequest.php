...
class UpdateDealRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'status' => ['sometimes', 'required', Rule::in(['open', 'won', 'lost'])],
            ...
            'source' => ['sometimes', 'required', Rule::in(['website_form', 'meta_ads', 'x', 'instagram', 'referral', 'manual', 'other'])],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Prevent changing status from won/lost back to open without proper reason
            $deal = $this->route('deal');
            if ($deal && in_array($deal->status, ['won', 'lost']) && $this->input('status') === 'open') {
                $validator->errors()->add('status', 'Cannot reopen a closed deal. Please create a new deal instead.');
            }
        });
    }
}
